...
class DiachronicCorpusCollector:
   def __init__(self):
       self.db_path = Path('corpus_new.db')  # Changed to corpus_new.db
       self.sources = {
           'perseus': {
   ...
   def generate_summary(self, body: str) -> str:
       '''Generate basic summary from first 3 sentences'''
       sentences = body.split('.')[:3]
       return '. '.join(sentences) + '.' if sentences else body[:200]
   ...
   def collect_from_source(self, source_name: str) -> List[Dict]:
       ...
       try:
           ...
           elements = soup.select(source['selector'])[:5]  # Limit to 5 for testing
           
           for element in elements:
               text_body = element.get_text(separator=' ', strip=True)
               
               if len(text_body) < 100:  # Skip very short texts
                   continue
               ...
               text_data = {
                   ...
                   'summary': self.generate_summary(text_body),
                   'body': text_body,
                   'language': 'Greek',
                   'source': source_name
               }
   ...
   def save_to_database(self, texts: List[Dict]):
       ...
       for text in texts:
           cur.execute('''
               INSERT INTO texts (title, author, period, source_url, full_description, summary, body, language)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?)
           ''', (
               text['title'],
               text['author'],
