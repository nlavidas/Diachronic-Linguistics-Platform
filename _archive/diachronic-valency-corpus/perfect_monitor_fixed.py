            ...
            for db_file in db_files:
                ...
                if os.path.exists(db_path):
                    ...
                    try:
                        ...
                        self.stats['texts_downloaded'] = count
                    except:
                        pass
                        
                    # Get total words if available
                    try:
                        cursor.execute('SELECT SUM(word_count) FROM texts WHERE word_count IS NOT NULL')
    ...
    def monitor_loop(self):
        ...
        while self.running:
            try:
                ...
                # Save monitor status (FIXED JSON serialization)
                try:
                    monitor_status = {
                        'timestamp': datetime.now().isoformat(),
                        'agent_running': self.find_agent_process() is not None,
                        'stats': {
                            'agent_restarts': self.stats['agent_restarts'],
                            'texts_downloaded': self.stats['texts_downloaded'],
                            'errors_handled': self.stats['errors_handled'],
                            'last_activity': self.stats['last_activity'].isoformat(),  # Convert to string
                            'uptime_hours': self.stats['uptime_hours']
                        }
                    }
                    
                    # Add total_words if available
                    if 'total_words' in self.stats:
                        monitor_status['stats']['total_words'] = self.stats['total_words']
                    
                    with open(f"{self.base_path}\\monitor_status.json", 'w') as f:
                        json.dump(monitor_status, f, indent=2)
                except:
                    pass  # Don't crash on JSON errors
