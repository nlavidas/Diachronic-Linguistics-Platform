...
try:
    ...
    logging.info("✅ spaCy available")
except:
    ML_TOOLS_AVAILABLE['spacy'] = False
    logging.info(⚠️ spaCy not installed - basic processing only")

# Check for transformers
try:
    from transformers import pipeline
    ML_TOOLS_AVAILABLE['transformers'] = True
    logging.info("✅ Transformers available")
except:
    ML_TOOLS_AVAILABLE['transformers'] = False
    logging.info("⚠️ Transformers not installed - no neural models")

# Check for NLTK
try:
    import nltk
    ML_TOOLS_AVAILABLE['nltk'] = True
    logging.info("✅ NLTK available")
except:
    ML_TOOLS_AVAILABLE['nltk'] = False
    logging.info("⚠️ NLTK not installed - no classical NLP")
...
class CompleteAutonomousAgent:
    ...
    def __init__(self):
        ...
        logging.info("✅ Complete agent initialized with GitHub integration")
...
- 🔄 Updates GitHub automatically every hour
...
"""
    ...
    def process_text_with_ml(self, text_id, filename):
        ...
        try:
            ...
            logging.info(f"✅ Processed: {filename} ({words:,} words, {tokens:,} tokens)")
...
def main():
    ...
    while True:
        try:
            ...
            restart_count += 1
            print(f"\n❌ Agent error: {e}")
            print(f"🔄 Restarting in 30 seconds... (attempt #{restart_count})")
